...
Save2Db(leveldb::DB *db,
  ...
  threads.emplace_back(std::thread([&db, &alg_hnsw]() {
  ...
  for (size_t i = 0; i < kLoadThreadCnt; ++i) {
    ...
    threads.emplace_back(std::thread([&db, &alg_hnsw, begin, end]() {
      char *data = nullptr;
      size_t data_size = 0;
      leveldb::WriteBatch wb;
      for (size_t i = begin; i < end; ++i) {
        GetHnswLinkListData(alg_hnsw, i, data, data_size);
        if (data_size > 0) {
          leveldb::Slice link_list_value(data, data_size);
          wb.Put(
              EncodeHnswLinkListKey(kTableId, kUin, kFieldId, kSubFieldId, i),
              link_list_value);
        }

        GetHnswLevel0Data(alg_hnsw, i, data, data_size);
        leveldb::Slice level0_data(data, data_size);
        wb.Put(
            EncodeHnswLevel0DataKey(kTableId, kUin, kFieldId, kSubFieldId, i),
            level0_data);
      }

      auto s = db->Write(leveldb::WriteOptions(), &wb);
      if (!s.ok()) {
        std::cout << "Failed to write db, " << s.ToString() << std::endl;
      }
    }));
...
int main(int argc, char **argv) {
  if (argc < 2) {
    std::cerr << "Usage: " << argv[0] << " [index_file_path] [load_thread_cnt]"
              << std::endl;
    return 0;
  }
  ...
  timer.Reset();
  auto delete_id = alg_hnsw->markDeleteByLabel(max_elements / 2);
  std::cout << "DELETE SINGLE KEY: total_cost=" << timer.ElapsedMicroseconds()
            << " μs, delete_id=" << delete_id << std::endl;

  // add single key
  std::string new_node = GetRandomVectorData(dim);
  // alg_hnsw->resizeIndex(alg_hnsw->max_elements_ + 1);
  timer.Reset();
  std::set<hnswlib::tableint> modify_ids;
  auto new_id = alg_hnsw->addPoint(
      (void *)new_node.data(), alg_hnsw->max_elements_ - 1, modify_ids, true);
  // save to db
  leveldb::WriteBatch wb;
  std::string meta_key =
      EncodeHnswMetaKey(kTableId, kUin, kFieldId, kSubFieldId);
  auto meta_value = GetHnswMetaValue(alg_hnsw);
  wb.Put(meta_key, meta_value.Serialize());

  modify_ids.insert(new_id);
  for (auto &it : modify_ids) {
    char *data = nullptr;
    size_t data_size = 0;
    GetHnswLinkListData(alg_hnsw, it, data, data_size);
    if (data_size > 0) {
      leveldb::Slice link_list_value(data, data_size);
      wb.Put(EncodeHnswLinkListKey(kTableId, kUin, kFieldId, kSubFieldId, it),
             link_list_value);
    }

    GetHnswLevel0Data(alg_hnsw, it, data, data_size);
    leveldb::Slice level0_data(data, data_size);
    wb.Put(EncodeHnswLevel0DataKey(kTableId, kUin, kFieldId, kSubFieldId, it),
           level0_data);
  }
  ...
  std::cout << "ADD SINGLE KEY: total_cost=" << timer.ElapsedMicroseconds()
            << " μs" << ", modify_ids_size=" << modify_ids.size()
